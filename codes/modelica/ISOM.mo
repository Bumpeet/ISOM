...
model hysys_comparision
  ...
  /*49*/3.87275E+33, 
  /*50*/5.86164E+30, 
  /*51*/5.99817E+22}"Pre exponential factor";
  ...
  /*49*/295.62, 
  /*50*/294.22,  
  /*51*/278.81}"activation energy";
   
  parameter Real Fi(unit="mol/hr") = 1000*1000;
  parameter Real yi[n] = {/*1 nC5*/0.032153001,
  /*2 iC5*/0.014040778,
  /*3 nC6*/0.018372195,
  /*4 2MP*/0.032489945,
  /*5 3MP*/0.021734323,
  ...
  /*15 C3*/ 0.003524092, 
  /*16 C2*/0.010245855, 
  /*17 CH4*/0.012111278} "inlet mole fraction ";
  parameter Real Ca1[n]=Fi*yi/695;
  ...
  parameter Real Ti (unit="K")= 130+273.15"inlet temperature";
  
  parameter Real mi[n](each unit = "gm/hr") = Fi*yi.*comp.MW "mass flowrate";
  parameter Real M(unit = "gm/hr") = sum(mi)"total mass flowrate";
  parameter Real xi[n] = mi/M "initial wt fraction";
  ...
  Real Cpigm, Cpm, delH_res_1[n];
  Real r_i[rxns](each unit = "Kmol/m3-sec"), Cpm_kg, T_in_c(unit="C");
...
// cp real calc
  ...
  delH_res_1 = 0.1*delH_ig;
  
  for i in 1:rxns loop
    if i <=18 then
      // calc overall heat for cooling  of reactants and heating of products for each reaction
      delH[i] = delH_ig[prod1[i]]-delH_ig[reac1[i]] + 0.1*delH_ig[prod1[i]]-0.1*delH_ig[reac1[i]];
    end if;
